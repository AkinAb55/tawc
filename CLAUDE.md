...
- **Build (gfxstream-bridge):** two scripts — `bash scripts/build-mesa-gfxstream.sh` (chroot-side `libvulkan_gfxstream.so` + ICD JSON, aarch64 glibc) and `bash scripts/build-gfxstream-backend.sh` (Android-side `libgfxstream_backend.so` + `libc++_shared.so`, NDK aarch64; staged into `app/src/main/jniLibs/arm64-v8a/`). The kumquat server itself is a Rust dep of the compositor (`compositor/Cargo.toml`'s `kumquat_virtio` entry, target-gated to aarch64), built as part of the cargo cross-compile and linked into `libcompositor.so`; it runs on a thread of the compositor process. Patches in `deps/gfxstream-patches/`, `deps/mesa-patches/`, `deps/rutabaga-patches/`. Gradle's `buildGfxstreamBackend` task wires the .so into `assembleDebug`. Stage the chroot-side bits with `bash scripts/bridge-setup.sh` (drops `libvulkan_gfxstream.so` + ICD into the rootfs via the broker — runs as the app uid, same SELinux context as the kumquat thread; SCM_RIGHTS fd-passing just works). Backend selection lives in the in-app Settings screen (`GraphicsBackend` enum in `Settings.kt`, persisted in `SharedPreferences`); `RootfsEnv` reads it on every rootfs spawn. Run the integration suite under either backend with `bash scripts/run-integration-tests.sh --graphics libhybris|gfxstream` (default `libhybris`); the runner flips the pref via the `set-graphics-backend` broker action and auto-stages the chroot-side bits when missing.
...
- **Integration tests:** `bash scripts/run-integration-tests.sh [--graphics libhybris|gfxstream] [filter]` (builds everything, deploys, runs all tests; the optional positional arg is a libtest substring filter, e.g. `apps::` or `apps::test_firefox`. `--graphics` flips the in-app graphics-driver pref before the suite starts (default `libhybris`) and exposes it to tests via `TAWC_GRAPHICS_BACKEND` so libhybris-only assertions can `SKIP:` cleanly. For `--graphics gfxstream` the runner stages the chroot-side `libvulkan_gfxstream.so` + ICD JSON via `bridge-setup.sh` if missing — the kumquat server itself runs in the compositor process, so there's no daemon to start.)

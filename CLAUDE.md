...
- **Build (gfxstream-bridge):** three scripts, in order — `bash scripts/build-mesa-gfxstream.sh` (chroot-side `libvulkan_gfxstream.so` + ICD JSON, aarch64 glibc), `bash scripts/build-gfxstream-backend.sh` (Android-side `libgfxstream_backend.so`, NDK aarch64), `bash scripts/build-kumquat-server.sh` (Android-side kumquat server binary, NDK aarch64). Patches in `deps/gfxstream-patches/`, `deps/mesa-patches/`, `deps/rutabaga-patches/`. Bring up end-to-end on the device with `bash scripts/bridge-setup.sh start` (does the build, pushes everything, starts the kumquat daemon under su, drops the libvulkan_gfxstream.so + ICD into the rootfs, chmod's the socket so the chroot can connect). See [notes/gfxstream-bridge.md](notes/gfxstream-bridge.md). Backend selection lives in the in-app Settings screen (`GraphicsBackend` enum in `Settings.kt`, persisted in `SharedPreferences`); `RootfsEnv` reads it on every rootfs spawn. Run the integration suite under either backend with `bash scripts/run-integration-tests.sh --graphics libhybris|gfxstream` (default `libhybris`); the runner flips the pref via the `set-graphics-backend` broker action before tests start.

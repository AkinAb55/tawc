...
- **Build (libxkbcommon):** auto-built per ABI by Gradle (`buildLibxkbcommon<Abi>`) ahead of the Rust compositor — same shape as libhybris. Standalone: `bash client/build-libxkbcommon [--abi=aarch64|x86_64|both] [--clean]`. Clones a pinned upstream tag into `./libxkbcommon/` (gitignored) if missing — no patches.

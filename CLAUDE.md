...
- **Build (gfxstream-bridge):** three scripts, in order — `bash scripts/build-mesa-gfxstream.sh` (chroot-side `libvulkan_gfxstream.so` + ICD JSON, aarch64 glibc), `bash scripts/build-gfxstream-backend.sh` (Android-side `libgfxstream_backend.so`, NDK aarch64), `bash scripts/build-kumquat-server.sh` (Android-side kumquat server binary as `app/src/main/jniLibs/arm64-v8a/libkumquat.so`, NDK aarch64; also stages `libgfxstream_backend.so` + `libc++_shared.so` alongside). Patches in `deps/gfxstream-patches/`, `deps/mesa-patches/`, `deps/rutabaga-patches/`. Gradle `buildBridge` task wires the kumquat build into `assembleDebug` so a fresh checkout's APK ships the daemon. Bring it up end-to-end on the device with `bash scripts/bridge-setup.sh start` (drops the libvulkan_gfxstream.so + ICD into the rootfs, then asks the broker to spawn kumquat as a child of the compositor app process — same SELinux context as the chroot client, which is required for SCM_RIGHTS fd-passing to work; see notes/gfxstream-bridge.md "Why kumquat must run as untrusted_app"). Backend selection lives in the in-app Settings screen (`GraphicsBackend` enum in `Settings.kt`, persisted in `SharedPreferences`); `RootfsEnv` reads it on every rootfs spawn. Run the integration suite under either backend with `bash scripts/run-integration-tests.sh --graphics libhybris|gfxstream` (default `libhybris`); the runner flips the pref via the `set-graphics-backend` broker action and auto-starts the kumquat daemon when missing.

...
"$TAWC_EXEC_BIN" /system/bin/sh -c "mkdir -p $build_dir && cp $TAWC_SCRATCH/$app_name.c $build_dir/$app_name.c && cp $TAWC_SCRATCH/$app_name-build.sh $build_dir/build.sh && chmod +x $build_dir/build.sh"

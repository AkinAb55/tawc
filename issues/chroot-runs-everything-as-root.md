...
- Files created in `/home/...` (Firefox profile, dotfiles, the user's
  ...
  `rootfs-run` or wires up file sharing in future, the
  ownership is wrong.
...
- **as-user** for user-launched apps: weston, Firefox, GTK clients,
  the interactive `rootfs-run` shell when the user types it.
...
- `scripts/rootfs-run.sh`: maybe a `--root` flag for the rare

...
`rootfs-run` shells into `<installation-dir>/enter.sh` over `adb shell
...
`rootfs-run` base64-encodes the chroot command into `enter.sh`'s `$1`,
...
- `install/` — Kotlin implementation of the chroot install / run /
  ...
  `scripts/rootfs-run.sh`, which shells into the same auto-generated

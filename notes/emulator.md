...
`TAWC_TARGET=emulator`. The host scripts (`run-integration-tests.sh`,
…) source `client/select-device.sh`, which sets `ANDROID_SERIAL`
accordingly. Pre-setting `ANDROID_SERIAL` overrides everything. With
only one target connected, `TAWC_TARGET` isn't needed.

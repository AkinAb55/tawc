...
### 6. Bootstrap the chroot
    TAWC_TARGET=emulator bash client/arch-chroot-create-emulator
This downloads the Arch x86_64 bootstrap tarball, pushes it to the
emulator, extracts to `/data/local/arch-chroot`, configures pacman, and
installs `base-devel` + Wayland + GTK3 inside the chroot. Takes a few
minutes the first time. Idempotent — re-running skips done steps.
...
Run something in the chroot:
    ...
    TAWC_TARGET=emulator adb push client/arch-chroot-run /data/local/tmp/
    TAWC_TARGET=emulator adb shell '/system/bin/sh /data/local/tmp/arch-chroot-run "uname -m"'

(Note `/system/bin/sh` instead of `/system_ext/bin/bash`. The emulator
has no bash in `/system_ext`, so `arch-chroot-run` is now POSIX/mksh
compatible and works under `/system/bin/sh` on both targets.)

## SELinux on the emulator
On a real device, `arch-chroot-run` uses `magiskpolicy --live` to install
...
- `client/arch-chroot-create-emulator` — host-driven counterpart of
  `arch-chroot-create` (which is Termux/device-driven). Does not need
  Termux on the AVD.
- `client/arch-chroot-run` — POSIX/mksh-compat. Same script for both
  targets. Invoked with `/system/bin/sh` (works everywhere) instead of
  `/system_ext/bin/bash` (real device only).
